...
let addonProto = {
  ...
  */
  shouldIncludeChildAddon(childAddon) {
    ...
    const seenAddonsPropName = isEngineDependency ? engineNamespace : 'seenAddonsForApp'
    ...
    if (this.project[seenAddonsPropName]) {
      ...
      if ((!childAddon.name.includes('cli-') && !childAddon.name.includes('lodash')) && seenVersion && semver.gte(seenVersion, childAddon.pkg.version)) {
