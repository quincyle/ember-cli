...
let addonProto = {
  ...
  */
  shouldIncludeChildAddon(childAddon) {
    ...
    const projectEmberAddon = this.project.pkg['ember-addon'];
    const isSymlink = childAddon.pkg._resolved === undefined;
    const childPackageVersion = isSymlink ? 'symlink' : childAddon.pkg.version;

    const seenAddonsPropName = this.project.pkg.keywords === 'ember-addon' ? 'seenAddonsForAddon' : 'seenAddonsForApp'
    ...
    if (this.project[seenAddonsPropName]) {
      ...
      if ((!childAddon.name.includes('cli-') && !childAddon.name.includes('lodash')) && seenVersion && (seenVersion === 'symlink' || semver.gte(seenVersion, childAddon.pkg.version))) {
        console.log(`Not including ${childAddon.name}, version ${childAddon.pkg.version} because we've seen ${seenVersion}`);
        return false;
      }
      this.project[seenAddonsPropName][childAddon.name] = childPackageVersion;
    } else {
      this.project[seenAddonsPropName] = {
        [childAddon.name]: childPackageVersion
      };
    }
    if (projectEmberAddon) {
