...
let addonProto = {
  ...
  */
  treeFor: function treeFor(treeType) {
    ...
    // let overwrite = true;
    // const projectEmberAddon = this.project.pkg['ember-addon'];
    // if (projectEmberAddon) {
    //   const onlyOverwrite = projectEmberAddon.onlyOverwrite;
    //   if (onlyOverwrite && onlyOverwrite[this.name]) {
    //     overwrite = onlyOverwrite[this.name] === this.pkg.version;
    //     console.log(`For ${this.name}, version ${this.pkg.version} overwriting will be set to ${overwrite}`);
    //   }
    // }
  ...
  */
  shouldIncludeChildAddon(childAddon) {
    ...
    const findEngineNameSpaceIfExists = function (parentAddon) {
      if (engineList.includes(parentAddon.name)) {
        return parentAddon.name
      }

      if (parentAddon.parent) {
        return findEngineNameSpaceIfExists(parentAddon.parent)
      }

      return false
    }

    // if (childAddon.name === 'ember-frost-table') debugger

    const engineNamespace = findEngineNameSpaceIfExists(this)

    // const engineNamespace = this.parent && typeof this.parent.name === 'string' && this.parent.name
    // const isEngineDependency = engineNamespace && engineList.includes(engineNamespace)

    // TODO: Determine if addon is a symlink and just always use that
    const projectEmberAddon = this.project.pkg['ember-addon'];
    const childPackageVersion = childAddon.pkg.version;

    const seenAddonsPropName = engineNamespace || 'seenAddonsForApp'

    if (this.project[seenAddonsPropName]) {
      const seenVersion = this.project[seenAddonsPropName][childAddon.name];
      // FIXME: remove hack to only do it for frost addons. Determine addons we need to keep (assuming cli addons)
      if ((childAddon.name.includes('frost') || childAddon.name.includes('smoke-and-mirrors')) && seenVersion && semver.gte(seenVersion, childAddon.pkg.version)) {
        console.log(`Not including ${childAddon.name} for ${engineNamespace}, version ${childAddon.pkg.version} because we've seen ${seenVersion}`);
        return false;
      }
      this.project[seenAddonsPropName][childAddon.name] = childPackageVersion;
    } else {
      this.project[seenAddonsPropName] = { [childAddon.name]: childPackageVersion };
    }
    if (projectEmberAddon) {
      const onlyUse = projectEmberAddon.onlyUse;
      if (onlyUse && onlyUse[childAddon.name]) {
        use = onlyUse[childAddon.name] === childAddon.pkg.version;


        console.log(`Including ${childAddon.name}, version ${childAddon.pkg.version}: ${use}`);
      }
    }
    return use;
  },
